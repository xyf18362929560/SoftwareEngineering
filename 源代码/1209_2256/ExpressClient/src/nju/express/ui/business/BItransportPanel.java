...
public class BItransportPanel extends JPanel {
	 ...
	 * Create the panel.
	 */
	public BItransportPanel(BItransportblservice bItransportblservice, Postblservice postblservice) {
	...
	private void query() {
		...
		for (Department department : departments) {
			if (department.getDepartment_name().equals(text)) {
				...
				String sql = "select * from bitransport where ( start_department_id_fk = ? or end_department_id_fk = ? ) and flag = 1 ";
		...
		if (isDatetime(text)) {
			...
			String sql = " select * from bitransport where bitransport_datetime > ?  and bitransport_datetime < ?  and flag = 1 ";
		...
		if (isTextExist() && isTextValidate()) {
			...
			if (result == 0) {
				...
				for (int i = 0; i < AllSelectedPostId.length; i++) {
					...
					String sql = "update post set bitransport_id_fk = ? where id = ? ";
	...
	private void update() {
		...
		if (result) {
			...
			for (Post post : selectedposts) {
				String sql = "update post set bitransport_id_fk = null where id = ? ";
	...
	public void initColumns() {
		...
		this.postcolumns.add("寄件人地址");
		this.postcolumns.add("收件人地址");
	...
	public void setViewDatas() {
		...
		String postsql = " SELECT * FROM post WHERE BItransport_id_fk is NULL ";
	...
	private BItransport getBItransport() {
		...
		return new BItransport(1, startDepartment.getId(), endDepartment.getId(),
	...
	private void addDepartment() {
		...
		Vector<Department> centers = bItransportblservice.getCenter();
		Department myDepartment = MainFrame.getInstance().getUser().getDepartment();

		Department itembusiness = makeDepartment(myDepartment);
		this.comboBox_startdepartment.addItem(itembusiness);
	...
	private void addVehicle() {
		...
		Vector<Vehicle> vehicles = bItransportblservice
				.getVehicle(((Department) this.comboBox_startdepartment.getSelectedItem()).getId());
	...
	private void addDriver() {
		...
		Vector<Driver> drivers = bItransportblservice
				.getDriver(((Department) this.comboBox_startdepartment.getSelectedItem()).getId());
